 ...
 */
public class SuggestedProductExtractor_New
{
    /**
    ...
    private static String callSentimentAPI(String serpApiResponse) throws Exception {
        ...
        String promptText = "\nAnalyze the following Walmart product search results and provide only the text output with the best product suggestion and its general review. Also create a review analysis in 2 lines.";
    ...
    /**
     * Formats the JSON response from SerpAPI into a structured HTML string.
     *
     * @param jsonResponse The JSON response from SerpAPI.
    ...
    private static String formatSerpApiResponse(String jsonResponse) {
        try {
            ...
            formatted.append("Top 10 Product Recommendations:\n\n");
    ...
    /**
     * Formats the JSON response from the Sentiment API into a structured HTML string.
     *
    ...
    private static String formatJsonResponse(String jsonResponse) {
        try {
            JSONObject jsonObject = new JSONObject(jsonResponse);
            StringBuilder formatted = new StringBuilder();
            formatted.append("=======OUR RECOMMENDATION:=======\n\n");
